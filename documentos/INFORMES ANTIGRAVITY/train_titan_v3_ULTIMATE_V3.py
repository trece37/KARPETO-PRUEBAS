...
from tensorflow.keras.layers import LSTM, Dense, Dropout, BatchNormalization, Bidirectional, Permute, Multiply, Flatten, Input, Concatenate, LayerNormalization
...
# --- LABELS (ADAPTIVE) ---
print("🛡️ APPLYING ADAPTIVE TRIPLE BARRIER...")
prices = df_merged['close'].values
volatility_for_labels = df_merged['volatility'].values
labels = apply_adaptive_triple_barrier(prices, volatility_for_labels, base_width=1.5, horizon=12)
...
# SCALER ROBUSTO (Solo en Train) - Upgrade #4 Implícito en buenas prácticas
scaler_m5 = RobustScaler()
scaler_d1 = RobustScaler()

print("⚖️ FITTING SCALERS (TRAIN ONLY)...")
train_m5_norm = scaler_m5.fit_transform(features_m5[:split_idx])
val_m5_norm = scaler_m5.transform(features_m5[split_idx:])
...
def build_surgical_model(input_shape_m5, input_shape_d1):
    ...
    x = Bidirectional(LSTM(128, return_sequences=True))(in_m5)
    x = Dropout(0.3)(x)
    x = LSTM(64)(x) # Last step only
    x = BatchNormalization()(x)
    
    # --- RAMA 2: D1 (Contexto) ---
    in_d1 = Input(shape=input_shape_d1, name='Input_D1') # Vector simple (features D1)
    y = Dense(32, activation='relu')(in_d1)
